        ...
        public void Report(Специальность specialty, Год_выпуска year)
        {
            using (db = new EntityContext())
            {
                ...
                try
                {
                    ...
                    sheet.Name = "Поступившие выпускники " + year.Номер_года;

                    sheet.Cells[1, 2] = "Поступившие выпускники " + year.Номер_года + "; Специальность - " + specialty.Название_специальности;
                    ...
                    sheet.Cells[2, 4] = "Наименование учебного заведения";
                    ...
                    var graduates = from q in db.Выпускники.Local
                                    where q.ID_Специальности == specialty.ID_Специальности &&
                                          ...
                                          q.Распределение.FirstOrDefault().Информация.FirstOrDefault(s => s.Номер_полугодия == 0).Должность == "Студент"
                                    orderby q.Фамилия
                                    select q;
                    int i = 3;
                    foreach (var item in graduates)
                    {
                        ...
                        sheet.Cells[i, 4] = item.Распределение.First().Куда_прибыл;
                        i++;
                    }

                    range = sheet.Range[sheet.Cells[2, 2], sheet.Cells[i - 1, 4]];
