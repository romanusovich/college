        ...
        public void Report(Специальность specialty, Год_выпуска year)
        {
            using (db = new EntityContext())
            {
                ...
                try
                {
                    ...
                    sheet.Name = "Список распределившихся " + year.Номер_года;

                    sheet.Cells[1, 2] = "Список распределившихся по собственному желанию " + year.Номер_года + "; Специальность - " + specialty.Название_специальности;
                    ...
                    var graduates = from q in db.Выпускники.Local
                                    where q.ID_Специальности == specialty.ID_Специальности &&
                                          ...
                                          q?.Средства_обучения?.Вид_средств_обучения == "Собственные средства" &&
                                          q.Распределение.First()?.Куда_прибыл != null
                                    orderby q.Фамилия
                                    select q;
                    int i = 3;
